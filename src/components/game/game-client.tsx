...
// Dialog modal showing bot's complete board details
function BotTableauDialog({ player, open, onClose }: { player?: PlayerState; open: boolean; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(val) => !val && onClose()}>
      <DialogContent className="border-white/5 bg-zinc-950 text-zinc-50 sm:max-w-2xl max-h-[85vh] overflow-y-auto shadow-2xl backdrop-blur-2xl">
...
// Compact Bot Portrait Card (under 95px height)
function BotCompactPanel({ player, active, onClick }: { player: PlayerState; active: boolean; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "rounded-xl border p-2 shadow-md transition-all duration-300 relative bg-zinc-950/30 flex flex-col justify-between h-[82px] cursor-pointer select-none",
        isMatchPoint
          ? "border-amber-500 bg-amber-950/15 shadow-[0_0_15px_rgba(245,158,11,0.3)] ring-1 ring-amber-500/30 animate-gold-pulse"
          : active
          ? "border-emerald-400/80 bg-zinc-900/40 shadow-emerald-500/5 ring-1 ring-emerald-500/20"
    ...
    >
      <div className="flex items-center justify-between gap-1.5">
        <div className="flex items-center gap-2 min-w-0">
          <div className={cn(
            "grid h-7 w-7 place-items-center rounded-full border bg-zinc-900 text-xs font-black shadow font-mono",
            isMatchPoint
          ...
          <div className="min-w-0">
            ...
            <p className="font-mono text-[9px] text-zinc-400 mt-1 leading-none">
              {player.hand.length} cards · ${bankTotal(player)}M bank
            </p>
          </div>
        </div>
        ...
        {completeSets > 0 && (
          <Badge className={cn(
            "font-bold px-1 py-0 h-4 border text-[9px] shrink-0",
            isMatchPoint
            ...
            {isMatchPoint ? "MATCH POINT" : `★ ${completeSets}`}
          </Badge>
        )}
      </div>

      {/* Property summary colors */}
      <div className="flex gap-1 overflow-x-auto scrollbar-none py-0.5 items-center">
        {groups.map((group) => (
          <span
            ...
            className={cn(
              ...
              group.complete ? "ring-1 ring-amber-400 font-black" : "font-bold"
            )}
            style={{
        ...
        {groups.length === 0 && (
          <span className="text-[9px] text-zinc-600 font-medium italic">No properties yet</span>
        )}
      </div>

      {active && (
        <span className="absolute -top-1 -left-1 grid h-3 w-3 place-items-center rounded-full bg-emerald-400 border border-zinc-950 animate-ping" />
      )}
    </div>
  );
}
...
export function GameClient() {
  ...
  const startNewGame = useCallback(
    (nextBots = botCount, nextDifficulty = difficulty) => {
      ...
      setSelectedPaymentIds([]);
      setMessage(undefined);
      setActiveBotMessage("Dealt fresh cards!");
  ...
  return (
    <main className="h-screen max-h-screen overflow-hidden bg-[radial-gradient(ellipse_at_50%_0%,rgba(16,185,129,0.22)_0%,rgba(6,25,18,0.95)_70%),linear-gradient(to_bottom,#020704,#090e0c_50%,#11090f)] text-zinc-50 flex flex-col justify-between p-2 sm:p-3 select-none">
      <div className="flex flex-col gap-2 h-full justify-between">
        ...
        {message && (
          <motion.div
            ...
            className="rounded-lg border border-rose-500/20 bg-rose-950/20 px-4 py-1.5 text-[11px] text-rose-300 flex items-center gap-2 shrink-0"
          >
        ...
        <section className="flex-1 min-h-0 flex flex-col gap-2 justify-between">
          ...
          <div className="grid gap-2 grid-cols-4 shrink-0 h-[82px] min-h-[82px]">
            ...
            {/* Fill empty spots if fewer than 4 bots to align layout */}
            {Array.from({ length: 4 - (state.players.length - 1) }).map((_, idx) => (
              <div key={idx} className="rounded-xl border border-dashed border-white/5 bg-black/5 opacity-20 h-full flex items-center justify-center text-[10px] text-zinc-600">
                Empty Spot
              </div>
            ))}
          </div>

          {/* LAYER 2: felt table center & sliding sidebar inspector */}
          <div className="flex-1 min-h-0 flex items-stretch gap-3">
            {/* Mahogany edge casino felt board */}
            <section className="relative overflow-hidden rounded-2xl border-[10px] border-zinc-950 bg-[radial-gradient(circle_at_50%_50%,rgba(16,185,129,0.18),rgba(5,20,15,0.92)_70%,rgba(3,8,6,0.98))] shadow-[inset_0_4px_25px_rgba(0,0,0,0.85),0_10px_35px_rgba(0,0,0,0.5)] flex-1 flex items-center justify-center">
              ...
              {activeBotMessage && (
                <div className="absolute top-4 left-1/2 -translate-x-1/2 rounded-full border border-emerald-400/20 bg-emerald-950/80 px-4 py-1.5 text-[11px] text-emerald-300 shadow-[0_0_15px_rgba(16,185,129,0.2)] animate-pulse flex items-center gap-1.5 z-20 font-semibold tracking-wide">
              ...
              {/* CENTER DECKS */}
              <div className="flex flex-col items-center justify-center gap-4.5 z-10 scale-90 sm:scale-100">
                <div className="flex items-center gap-8 justify-center">
                  ...
                  <div className="text-center flex flex-col items-center">
                    <div className="relative">
                      {state.discard.at(-1) ? (
                        <>
                          <CardView card={state.discard.at(-1)} size="md" className="shadow-2xl relative border border-white/5" />
                          <span className="absolute -bottom-1 -right-1 grid h-5 w-7 place-items-center rounded bg-zinc-950 border border-white/10 text-[10px] font-black font-mono">
                ...
                <div className="flex gap-2 bg-black/55 border border-white/10 p-1.5 rounded-xl shadow-2xl backdrop-blur-xl">
                  {drawMove && (
                ...
                {state.pendingInteraction && (
                  ...
                  >
                    <motion.div
                      ...
                      className="bg-zinc-950/95 border border-emerald-500/20 p-5 rounded-2xl max-w-xs w-full text-center flex flex-col items-center shadow-[0_0_50px_rgba(16,185,129,0.25)] animate-none"
                    >
            ...
            {/* CARD INSPECTOR (opens on the right on desktop, or floating dock on mobile) */}
            <AnimatePresence>
              {selectedCard && (
                isMobile ? (
                  <motion.div
                    ...
                    className="fixed bottom-[265px] left-3 right-3 z-30 flex flex-row items-center gap-3 rounded-2xl border border-emerald-500/20 bg-zinc-950/95 p-3 shadow-2xl backdrop-blur-xl justify-between overflow-hidden"
                  >
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between h-full">
                        <div className="relative">
                          ...
                          <button
                            ...
                            className="absolute right-0 top-0 p-1 hover:bg-white/5 rounded text-zinc-400 hover:text-zinc-200 shrink-0 cursor-pointer"
                          >
                        ...
                        <div className="flex gap-1.5 mt-2 overflow-x-auto scrollbar-none py-0.5">
                          {selectedMoves.map((move, index) => (
                            <Button
                              key={`${move.type}-${index}`}
                              size="sm"
                              className="text-[10px] h-7 px-2.5 font-bold bg-emerald-600 hover:bg-emerald-500 text-white cursor-pointer shrink-0"
                          ...
                          {selectedMoves.length === 0 && (
                            <p className="text-[9px] text-zinc-500 italic bg-black/25 px-2.5 py-1 rounded shrink-0">
                  ...
                  <motion.aside
                    ...
                    animate={{ width: 310, opacity: 1, marginLeft: 12 }}
                    exit={{ width: 0, opacity: 0, marginLeft: 0 }}
                    transition={{ type: "spring", stiffness: 300, damping: 26 }}
                    className="shrink-0 flex flex-col rounded-2xl border border-white/5 bg-zinc-950/70 p-4 shadow-xl justify-between overflow-hidden relative"
                  >
                    <div className="absolute inset-0 bg-[radial-gradient(circle_at_bottom_right,rgba(16,185,129,0.02),transparent_40%)]" />
                    <div className="flex-1 flex flex-col justify-between h-full min-w-0 z-10">
                      <div>
                        <div className="flex justify-between items-center border-b border-white/5 pb-2">
                          <span className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider text-zinc-300">
                      ...
                      <div className="grid gap-1.5 mt-3 pt-3 border-t border-white/5">
                        ...
                        {selectedMoves.length === 0 && (
                          <p className="text-[9px] text-zinc-500 italic bg-black/25 p-2 rounded text-center">
                            No actions available in this turn phase.
                          </p>
                        )}
                      </div>
          ...
          {/* LAYER 3: COMPACT PLAYER PANEL & HAND (260px) */}
          <section className="rounded-2xl border border-white/5 bg-black/20 p-2.5 shadow-2xl shrink-0 h-[255px] min-h-[255px] flex flex-col justify-between overflow-hidden">
            {/* Player Stats & Tableau row */}
            <div className="flex items-center gap-3 min-h-[110px] h-[110px] min-w-0">
              {/* Stats card */}
              <div className="w-[10.5rem] rounded-xl border border-white/5 bg-zinc-950/40 p-2 flex flex-col justify-between shrink-0 h-full">
                <div className="flex items-center gap-2">
                  <div className="grid h-8 w-8 place-items-center rounded-full border border-emerald-500/20 bg-emerald-950/15 font-mono text-sm font-black text-emerald-300">
                    {human.avatar}
                  </div>
                  <div className="min-w-0">
                ...
                <div className="flex items-center justify-between border-t border-white/5 pt-1.5 mt-1">
                  <div className="flex flex-col">
                    <span className="text-[9px] font-black uppercase text-zinc-500 tracking-wider">Vault</span>
                    <span className="font-mono text-xs font-black text-amber-300">${bankTotal(human)}M</span>
                  </div>
                  <div className="flex -space-x-4 overflow-visible max-w-[65px] shrink-0">
              ...
              <div className="flex-1 flex gap-2 overflow-x-auto scrollbar-thin items-stretch h-full pb-1">
                {humanGroups.map((group) => (
                  <SetColumn key={group.color} group={group} small={true} />
                ))}
                {humanGroups.length === 0 && (
                  <div className="flex-1 rounded-xl border border-dashed border-white/5 bg-black/10 flex items-center justify-center text-xs text-zinc-500 italic">
                    You have no properties played yet.
                  </div>
                )}
              </div>
            </div>
            ...
            {/* Hand row (Viewport-Locked fanned hand) */}
            <div className="overflow-x-auto scrollbar-none h-[135px] min-h-[135px] shrink-0 flex items-end justify-center pb-1">
              <div className="flex items-end justify-center -space-x-5 hover:-space-x-1.5 px-6 transition-all duration-300">
      ...
      {/* DETACHED BOT TABLEAU DETAIL DIALOGS */}
